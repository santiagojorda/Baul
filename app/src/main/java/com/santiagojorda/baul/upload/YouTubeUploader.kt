...
class YouTubeUploader(
    ...
    override suspend fun upload(file: MediaFile, rule: Rule, onProgress: (bytesSent: Long) -> Unit): UploadResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val insertRequest = youtube.videos().insert(listOf("snippet", "status"), video, mediaContent)
            insertRequest.mediaHttpUploader.setProgressListener { uploader -> onProgress(uploader.numBytesUploaded) }
            val insertedVideo = insertRequest.execute()
